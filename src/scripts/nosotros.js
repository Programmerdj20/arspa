...
function openBookingModal() {
    // Llamar a la función global del modal si existe
    if (window.openBookingModal && typeof window.openBookingModal === "function") {
        // Verificar que no sea la misma función para evitar recursión
        if (window.openBookingModal !== openBookingModal) {
            window.openBookingModal();
            return;
        }
    }

    // Fallback a WhatsApp si el modal no está disponible
    const message = encodeURIComponent(
        "¡Hola! Me interesa agendar una cita en ARspa."
    );
